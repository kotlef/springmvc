 ...
 */
public abstract class AbstractRsaEncryption {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(AbstractRsaEncryption.class);

	/** 算法名称 */
	private static final String ALGORITHOM = "RSA";

	private String algorithom = ALGORITHOM;
	/** 保存生成的密钥对的文件名称。 */
	private static final String RSA_PAIR_FILENAME = "/__RSA_PAIR.txt";

	private String rsaPairFileName = RSA_PAIR_FILENAME;
	/** 密钥大小 */
	...
	private static boolean RSA_MEMORY_OR_FILE = false;

	private boolean rsaMemoryOrFile = RSA_MEMORY_OR_FILE;
	...
	public String getAlgorithom() {
		return algorithom;
	}

	public void setAlgorithom(String algorithom) {
		this.algorithom = algorithom;
	}

	public String getRsaPairFileName() {
		return rsaPairFileName;
	}

	public void setRsaPairFileName(String rsaPairFileName) {
		this.rsaPairFileName = rsaPairFileName;
	}
	...
	public static File getRsaPairFile() {
		return rsaPairFile;
	}

	public static void setRsaPairFile(File rsaPairFile) {
		AbstractRsaEncryption.rsaPairFile = rsaPairFile;
	}

	public boolean isRsaMemoryOrFile() {
		return rsaMemoryOrFile;
	}

	public void setRsaMemoryOrFile(boolean rsaMemoryOrFile) {
		this.rsaMemoryOrFile = rsaMemoryOrFile;
	}
	...
	public synchronized KeyPair generateKeyPair() {
		try {
			...
			LOGGER.error(
					"AbstractRsaEncryption#KEY_PAIR_GEN is null, can not generate KeyPairGenerator instance.",
					ex);
		}
		return null;
	}
	...
	public RSAPublicKey generateRSAPublicKey(byte[] modulus,
		...
		try {
			...
			LOGGER.error(
					"AbstractRsaEncryption#KEY_FACTORY is null, can not generate KeyFactory instance.",
					ex);
		}
		return null;
	}
	 ...
	 * @return RSA专用私钥对象。
	 */
	public RSAPrivateKey getRSAPrivateKey(String hexModulus,
	 ...
	 * @return RSA专用公钥对象。
	 */
	public RSAPublicKey getRSAPublidKey(String hexModulus,
	 ...
	 * @return 给定字符串的密文。
	 */
	public String encryptString(KeyPair keyPair, String plaintext) {
		if (plaintext == null) {
			return null;
		}
		byte[] data = plaintext.getBytes();
	 ...
	 */
	public String decryptStringByJs(KeyPair keyPair, String encrypttext) {
		...
		try {
			...
			byte[] raw = decrypt((RSAPrivateKey) keyPair.getPrivate(), en_data);
	 ...
	 * @return {@code encrypttext} 的原文字符串。
	 */
	public String decryptStringReverseByJs(KeyPair keyPair, String encrypttext) {
		String text = decryptString(keyPair, encrypttext);
		if (text == null) {
			return null;
		}
	...
	/** 返回已初始化的默认的公钥。 */
	public RSAPublicKey getDefaultPublicKey(KeyPair keyPair) {
		if (keyPair != null) {
	...
	/** 返回已初始化的默认的私钥。 */
	public RSAPrivateKey getDefaultPrivateKey(KeyPair keyPair) {
	...
	public PublicKeyMap getHexPublicKeyMap(RSAPublicKey rsaPublicKey) {
	...
	public PublicKeyMap getBase64PublicKeyMap(RSAPublicKey rsaPublicKey) {
		PublicKeyMap publicKeyMap = new PublicKeyMap();
		publicKeyMap.setModulus(new String(BASE64Encoder.encode(rsaPublicKey
				.getModulus().toByteArray())));
		publicKeyMap.setExponent(new String(BASE64Encoder.encode(rsaPublicKey
				.getPublicExponent().toByteArray())));
		return publicKeyMap;
	}

	public PublicKeyMap getJsPublicKeyMap(PublicKey publicKey) {
		JCERSAPublicKey jce = (JCERSAPublicKey) publicKey;

		PublicKeyMap publicKeyMap = new PublicKeyMap();
		publicKeyMap.setModulus(jce.getModulus().toString(16));
		publicKeyMap.setExponent(jce.getPublicExponent().toString(16));
		return publicKeyMap;
	}
